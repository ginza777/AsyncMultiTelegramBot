...
from apps.bot.models import TelegramBot, TelegramProfile
import telegram
...
async def register_user_if_not_exists(update: Update, context: CallbackContext, tg_userid: int):
    ...
    if tg_userid not in user_semaphores:
        user_semaphores[tg_userid] = asyncio.Semaphore(1)

    if user.current_model is None:
        user.current_model =   models["available_text_models"][0]
...
async def generate_image_handle(update: Update, context: CallbackContext, message=None):
    ...
    # token usage
    db.set_user_attribute(user_id, "n_generated_images",   return_n_generated_images + db.get_user_attribute(user_id, "n_generated_images"))
